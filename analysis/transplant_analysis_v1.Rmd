...
phenotypes <- c("RRT_length", "cold_ischemia_time", "DGF", "immunological_risk", "sensitized", "rejection", "viral_complication", "bacterial_complication", "fungal_complication", "number_HLAmm", "number_DRmm", "CMV_status", "CMV_IgG", "K_preTx", "IgG_level", "GFR_preTx", "CRP", "t_cell_immunophenotyping")
...
par(mar = c(8, 7.5, 3, 3));
labeledHeatmap(Matrix = display_corr, xLabels = xlabs, yLabels = substring(names(MEs), first = 3), ySymbols = names(MEs), colorLabels = FALSE, colors = blueWhiteRed(50), textMatrix = textMatrix, setStdMargins = FALSE, cex.text = 0.5, zlim = c(-1,1), main = paste("Module-trait relationships"), cex.lab.x = 0.75)
dev.off()
...
xlabs[which(colSums(discrete_p < 0.01) > 0)] = paste0(xlabs[which(colSums(discrete_p < 0.01) > 0)], "*")

# Display the correlation values within a heatmap plot
pdf("../results/discrete_corr_heatmap.pdf")
par(mar = c(7, 7.5, 3, 3));
labeledHeatmap(Matrix = discrete_corr, xLabels = xlabs, yLabels = substring(names(MEs), first = 3), ySymbols = names(MEs), colorLabels = FALSE, colors = blueWhiteRed(50), textMatrix = textMatrix, setStdMargins = FALSE, cex.text = 0.5, zlim = c(-1,1), main = paste("Module-trait relationships for Discrete Clinical Outcomes"))
dev.off()
